...
sub from_store {
    ...
    my $user      = config->{intercom_user};
    my $pass      = config->{intercom_pass};
    my $host      = config->{intercom_host};
    my $timeout   = config->{intercom_timeout};

    if ( !( $user && $pass && $host && $timeout ) ) {
        $logger->error('Missing SM RPC settings from config.yml');
        return;
    }
